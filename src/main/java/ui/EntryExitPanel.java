...
public class EntryExitPanel extends JPanel {
    ...
    private void configureInputs() {
        ...
        AppTheme.styleTextField(cashField);
        AppTheme.styleTextField(cardField);

        cashField.setToolTipText("e.g., 20.00");
        cardField.setToolTipText("12–19 digits");
    ...
    private JPanel buildExitPanel() {
        ...
        JPanel paymentChoice = new JPanel(new GridLayout(1, 2, 8, 0));
        paymentChoice.setOpaque(false);
        paymentChoice.add(cashRadio);
        paymentChoice.add(cardRadio);
        form.addRow("3) Method", paymentChoice);

        JPanel paymentInput = new JPanel(new GridLayout(1, 2, 8, 0));
        paymentInput.setOpaque(false);
        paymentInput.add(cashField);
        paymentInput.add(cardField);
        form.addRow("4) Cash / Card", paymentInput);
    ...
    private void processPayment() {
        ...
        try {
            if (method == PaymentMethod.CASH) {
                ...
                cardNumber = cardField.getText().trim();
            }
        } catch (Exception ex) {
